...
winner simulationManager::updateSimulation(int timestep) {
    ...
    ///@details handles calling sav or destroying it
    if (getEarthSoldiersCount() != 0 &&
        (earthArmyPtr->getSAVstatus() == hasntCallSAV &&
         double(getEarthInfectedSoldierCount()) / getEarthSoldiersCount() * 100 >= getCallSAVPer()) &&
        currentTimeStep > 150)
        earthArmyPtr->setSAVstatus(SAVBeingUsed);

    if ((double(getEarthInfectedSoldierCount()) / getEarthSoldiersCount()) * 100 <= 5 &&
...
void simulationManager::choose_Mode() {
    cout << "choose Mode of fight \n1- Interactive Mood \n2- Silent Mode \n";
    int choice{0};
    cin >> choice;
    switch (choice) {
        case 1:

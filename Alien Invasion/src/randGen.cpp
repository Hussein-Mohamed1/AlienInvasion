...
void randGen::manageGeneration(fstream *infile) {
    ...
    if (num <= prob) //generate units TODO waiting noser
    {
        ...
        generatUnits(perAM, RAH1, RAH2, RAP1, RAP2, RAC1, RAC2, MonsterType); // add alien monster
        perAD = handelPer(perAD, unitscreated);// handel num of units created depening on persentage
        generatUnits(perAD, RAH1, RAH2, RAP1, RAP2, RAC1, RAC2, DronePair); // add alien Drone
    }
    num = rand() % 100;
    if (num > 0 && num < 10) {
